...
function Pattern(props) {
    ...
    function highlightRow(row) {
        ...
        for (let i = 0; i < rp.length; i++) {
            rp[i].style.backgroundColor = '#FFFFFF'
        }
    ...
    function getPattern() {
        ...
        for (let i = 1; i <= height; i = i + counter) {
            ...
            for (let j = 1; j <= width; j = j + counter) {
                ...
                let numInRow=j;
                pix.push(<div className='patternPix' id={idx}style={{backgroundColor:colors[loc], width: '20px', height: '20px', border:'1px solid black'}} >{numInRow}</div>);
                loc += counter;

            }
            // loc += counter;
            row.push(<div className='rowNum'id={idy}   style={{ backgroundColor: '#FBFFF2'}}>{pix}</div>)
        }
        pattern.push(row)
        return (pattern)
    }
